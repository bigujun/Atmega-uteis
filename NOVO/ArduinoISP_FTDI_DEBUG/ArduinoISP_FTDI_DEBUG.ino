...
#define BAUD_ISP 19200
#define BAUD_FTDI 115200
...
void loop()
{
    ...
    if (!inBreak() && SerialUSB.baud() == BAUD_ISP){ loopISP();}
    else{ loopFTDI();}
...
void empty_reply() {
  if (CRC_EOP == getch()) {
    SerialUSB.print((char)STK_INSYNC);
    SerialUSB.print((char)STK_OK);
  } 
  else {
    error++;
    SerialUSB.print((char)STK_NOSYNC);
  }
}
...
void write_flash(int length) {
  ...
  if (CRC_EOP == getch()) {
    SerialUSB.print((char) STK_INSYNC);
    SerialUSB.print((char) write_flash_pages(length));
  } 
  else {
    error++;
    SerialUSB.print((char) STK_NOSYNC);
...
void program_page() {
  ...
  SerialUSB.print((char)STK_FAILED);
  return;
}
...
char flash_read_page(int length) {
  for (int x = 0; x < length; x+=2) {
    ...
    SerialUSB.print((char) low);
    uint8_t high = flash_read(HIGH, here);
    SerialUSB.print((char) high);
    here++;
  }
  return STK_OK;
}
...
int avrisp() { 
  ...
  default:
    error++;
    if (CRC_EOP == getch()) 
      SerialUSB.print((char)STK_UNKNOWN);
    else
      SerialUSB.print((char)STK_NOSYNC);
  }
}
